...
![macOS 安全提示放行](docs/images/macos-gatekeeper-open.png)

> 上图是 macOS 系统设置 → 隐私与安全性里的提示。点"仍要打开"后，应用即可正常启动；此操作只需一次，之后双击即可直接打开。
...
> 向导会自动检测本机已安装的 dsh（源码 / npm 全局 / 本地），选中后填端口号（默认 3080，填 0 = 自动分配），点"保存并启动"即可。检测不到时，向导会给出安装指引。
